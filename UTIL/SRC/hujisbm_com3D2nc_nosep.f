...
c
	...
	read(c_dx,'(f10.0)') dx
	read(c_dy,'(f10.0)') dy
	read(c_time,'(f10.4)') time
	do k=1,nz
		read(c_z(k),'(f10.3)') z(k)
	end do
	do k=1,nz
		read(c_p(k),'(f10.3)') p(k)
	end do
	read(1) (c_rn(k),k=1,ncn),(c_rnr(k),k=1,ncn)
	do k=1,ncd
		read(c_rn(k),'(e10.4)') rccn(k)
	end do
	do k=1,ncd
		read(c_rnr(k),'(e10.4)') rcd(k)
	end do
...
c
	...
	do while(ifields.lt.nfields)
     ...
     &						fmin+(byte(count)+32000.)*(fmax-fmin)/64000.
		...
		err = NF_PUT_ATT_TEXT(ncid,varid,'long_name',strlen1(long_name),long_name(1:strlen1(long_name)))
		if (err .ne. NF_NOERR) print*, NF_STRERROR(err)
		err = NF_PUT_ATT_TEXT(ncid,varid,'units',strlen1(units),units(1:strlen1(units)))
		if (err .ne. NF_NOERR) print*, NF_STRERROR(err)
		err = NF_ENDDEF(ncid)
