...
c variables:
	...
	character (10) c_z(500),c_p(500),c_rho(500), c_dx, c_dy, c_time	
	character(3) nm

	integer(2), allocatable :: byte(:)
	real(4), allocatable ::  fld(:)
	real(4) fmax,fmin
	real(4) dx,dy,z(500),p(500),rho(500),x(100000),y(100000),time
	...
	integer ncn, ncd
	character(10) c_rn(50),c_rnr(50)
	real(8) rccn(50), rdrop(50)
...
c
	...
	read(long_name,'(9i4)') nx,ny,nz,ncn,ncd,nsubs,nsubsx,nsubsy,nfields
	read(1) c_time,c_dx,c_dy,(c_z(k),k=1,nz),(c_p(k),k=1,nz),(c_rho(k),k=1,nz)
	print*,c_time,c_dx,c_dy,(c_z(k),k=1,nz),(c_p(k),k=1,nz), (c_rho(k),k=1,nz)
	read(c_dx,'(f10.0)') dx
	read(c_dy,'(f10.0)') dy
	...
	do k=1,ncd
      read(c_rnr(k), '(e10.4)') rdrop(k)
	end do

	print*, (rccn(k), k=1, ncn)
	print*, (rdrop(k), k=1, ncd)

	print*,'nx,ny,nz,ncn,ncd,nsubs,nsubsx,nsubsy,nfields:'
	print*,nx,ny,nz,ncn,ncd,nsubs,nsubsx,nsubsy,nfields
