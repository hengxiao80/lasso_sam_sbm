...
c variables:
      ...
      character(10) c_z(600), c_p(600), c_rho(600), c_dx, c_dy, c_time
      character(10) c_rccn(nbin), c_rcd(nbin)
      character(4) rankchar
      integer(2), allocatable :: byte(:)
      real dx,dy,z(600),p(600),time
      integer nsubs,nsubsx,nsubsy,nx,ny,nz,nfields,ncn,ncd
...
c
      ...
      read(1,end=3333,err=3333) long_name(1:40)
      write(3) long_name(1:40)
      print*,long_name(1:40)
      read(long_name,'(10i4)') nx,ny,nz,ncn,ncd,nsubs,nsubsx,nsubsy,nfields
      read(1) c_time,c_dx,c_dy,(c_z(k),k=1,nz),(c_p(k),k=1,nz), (c_rho(k), k=1,nz)
      read(1) (c_rccn(k),k=1,ncn),(c_rcd(k),k=1,ncn)
      print*, c_time,c_dx,c_dy
      write(*, '(3A10)') (c_z(k), c_p(k), c_rho(k), k=1,nz)
      write(*, '(2A10)') (c_rccn(k), c_rcd(k), k=1,ncn)
      ...
      do ifields=0,nfields-1
        do n=0,nsubs-1
          if(n.ne.0) then
            write(rankchar,'(i4)') n
            open(2,file=trim(filename)//"_"//trim(adjustl(rankchar)), status='old',form='unformatted')
            do i = 1,ifields
              read(2)
            end do
            read(2) (byte(k),k=1,nx*ny*nz)
          else
            read(1) name,blank,long_name,blank,units,blank,(c_max(k),k=1,nz),(c_min(k),k=1,nz)
            write(3) name,blank,long_name,blank,units,blank,(c_max(k),k=1,nz),(c_min(k),k=1,nz)
