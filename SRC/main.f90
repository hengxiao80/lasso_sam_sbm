...
nstatsteps = 0
!-------------------------------------------------------------------
! Write initial profiles for ISDAC (MO: mikhail@pnnl.gov)
!-------------------------------------------------------------------
 if(nrestart.eq.0) then         !MO 4/19/16
   call statistics()
   call micro_statistics()
   call sgs_statistics()        !MO 4/19/16
   ...
   call hbuf_flush()	  
   nstatsteps = 0
 end if      ! (nrestart.eq.0)  !MO 4/19/16
!-------------------------------------------------------------------
! End write initial profile 
!------------------------------------------------------------------
...
!MO 4/19/16: For SBM, precipitation is controlled by docoag flag in 
!            microphysics.f90. tprecip is set in params.f90
!            There tprecip is in seconds
!MO: This controls precipitation when using M2005; tpreip is in days  
!  if (day.lt.tprecip) then ! For VOCALS to call precip after 1h
!      doprecip = .false.
!  else
!      doprecip = .true.
!  endif
