...
! added for SHEBA output from radar simulator
      V_doppiz(nz), Zez(nz)   
...
!=============================================================
! Added for ISDAC intercomparison statistics (MO: Feb 2012, mikhail@pnnl.gov)

real &
    zinv,    &  ! inversion height (thetal crossing 267 K)
    zctl,    &  ! liquid cloud-top height (ql>0.0001 g/kg)
    zcbl,    &  ! liquid cloud-base height (qi>0.0001 g/kg)
    lwpa,    &  ! mean liquid water path
    iwpa,    &  ! mean ice water path
    lwp_var, &  ! liquid water path variance
    iwp_var, &  ! ice water path variance
    prl_sfc, &  ! liquid precipitation flux at the surface
    pri_sfc, &  ! ice precipitation flux at the surface
    prl_400, &  ! liquid precipitation flux at z=400m
    pri_400     ! ice precipitation flux at z=400m

real &
    cf_lz(nzm),   & ! Liquid cloud fraction
    cf_iz(nzm),   & ! Ice cloud fraction
    wup(nzm),     & ! Mean updraft velocity (m/s)
    wdn(nzm),     & ! Mean downdraft velocity (m/s)
    wup_fr(nzm),  & ! Updraft areal fraction ()
    wdn_fr(nzm),  & ! Downdraft areal fraction ()
    radlwnt(nz),  & ! net longwave radiative flux
    radswnt(nz)     ! net shortwave radiative flux

! END Added for ISDAC intercomparison statistics
!===========================================================
