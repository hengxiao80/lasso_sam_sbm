...
implicit none	
	...
	real tpz(nzm)
	real tlz(nzm)
	real tilz(nzm)    ! liquid-ice water potential temperature (MO)
	real tvz(nzm)
	real qcz(nzm)
	real qiz(nzm)
	real tez(nzm)
	...
	real tmp_out(nzm)    ! MO 4/13/16 Temp. array for buffer to avoid
                             ! compiler warning for a+b  type arguments  
	real tmp_out3d(nx,ny,nzm)        
...
!mo           qiz(k)=qiz(k)+qii
	   ...
	   tmp(1)=tabs(i,j,k)*prespot(k)
           tpz(k)=tpz(k)+tmp(1)
!mo	   tlz(k)=tlz(k)+tmp(1)*(1.-fac_cond*(qcl(i,j,k)+qci(i,j,k))/tabs(i,j,k))
	   tlzz=tmp(1)*(1.-fac_cond*(qcl(i,j,k)+qpl(i,j,k))/tabs(i,j,k))  !mo
	   tlz(k)=tlz(k)+tlzz                                             !mo
!======================================================================
! added inversion level for ISDAC (MO)
!MO4/13/16           if (tlzz > 267. .AND. zinv_xy(i,j)==0.) then                   !mo
! added inversion level for ISDAC (MO)
!           if (tlzz > 289. .AND. zinv_xy(i,j)==0.) then                   !mo MO4/13/16
!              kb = max(k-1, 1)                                            !mo
!              zinv_xy(i,j)= z(k) -(z(k)-z(kb))*(tlzz-267.)/   &           !mo
!                    (tlzz-tabs(i,j,kb)*prespot(kb)*           &           !mo
!                    (1.-fac_cond*(qcl(i,j,kb)+qpl(i,j,kb))/tabs(i,j,kb))) !mo 
!           end if                                                         !mo
!======================================================================
           tilz(k)=tilz(k)+tmp(1)*(1.-(fac_cond*(qcl(i,j,k)+qpl(i,j,k))+  &  !mo
                           fac_sub*(qci(i,j,k)+qpi(i,j,k)))/tabs(i,j,k))     !mo
...
!MO	call hbuf_avg_put('U',u+ug,dimx1_u,dimx2_u,dimy1_u,dimy2_u,nzm,1.)
!MO	call hbuf_avg_put('V',v+vg,dimx1_v,dimx2_v,dimy1_v,dimy2_v,nzm,1.)
!MO	call hbuf_avg_put('QT',qv+qcl+qci,1,nx,1,ny,nzm,1.e3)
        tmp_out3d(1:nx,1:ny,1:nzm)=u(1:nx,1:ny,1:nzm)+ug            !MO 4/13/16
	call hbuf_avg_put('U',tmp_out3d,1,nx,1,ny,nzm,1.)           !MO 4/13/16
        tmp_out3d(1:nx,1:ny,1:nzm)=v(1:nx,1:ny,1:nzm)+vg	    !MO 4/13/16	
	call hbuf_avg_put('V',tmp_out3d,1,nx,1,ny,nzm,1.)           !MO 4/13/16
        tmp_out3d(1:nx,1:ny,1:nzm)=qv(1:nx,1:ny,1:nzm)+          &
                          qcl(1:nx,1:ny,1:nzm)+qci(1:nx,1:ny,1:nzm) !MO 4/13/16	
	call hbuf_avg_put('QT',tmp_out3d,1,nx,1,ny,nzm,1.e3)
	...
	call hbuf_put('THETAIL',tilz,factor_xy)
...
!mo	call hbuf_put('RELH',relhz,100.*factor_xy)
	call hbuf_put('RELH',relhz,factor_xy)  ! saturation ratio
...
!-------------------------------------------------------------
	...
	do k=1,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    tvz(k) = tvz(k) + tvirt(i,j,k)
!mo	    qcz(k) = qcz(k) + qcl(i,j,k)
!mo	    qiz(k) = qiz(k) + qci(i,j,k)
	    qcz(k) = qcz(k) + qcl(i,j,k) + qpl(i,j,k) ! Include precip into liquid 
	    qiz(k) = qiz(k) + qci(i,j,k) + qpi(i,j,k) ! and ice profiles for ISDAC
	    qsatwz(k) = qsatwz(k)+qsatw(tabs(i,j,k),pres(k))
	  end do
	 end do
...
!mo	  call task_sum_real(buffer,buffer1,nzm*4)
	  call task_sum_real(buffer,buffer1,nzm*5)   ! mo
	  do k=1,nzm
	    tvz(k) = buffer1(k,1) * coef1
	    qcz(k) = buffer1(k,2) * coef1
	    qiz(k) = buffer1(k,3) * coef1
	    qsatwz(k) = buffer1(k,4) * coef1
	    tlz(k) = buffer1(k,5) * factor_n         ! mo
	  end do
	end if ! dompi
...
!mo	    qcwle(k) = qcwle(k) + 0.5*w(i,j,k)* &
!mo		(qcl(i,j,k-1)-qcz(k-1)+ qcl(i,j,k)-qcz(k))	  
!mo	    qiwle(k) = qiwle(k) + 0.5*w(i,j,k)* &
!mo                (qci(i,j,k-1)-qiz(k-1)+qci(i,j,k)-qiz(k)) 
	    qcwle(k) = qcwle(k) + 0.5*w(i,j,k)*      &  ! Include precip into the  
		( qcl(i,j,k-1)+qpl(i,j,k-1)-qcz(k-1) &  ! profiles for liquid and
                 +qcl(i,j,k)+qpl(i,j,k)-qcz(k))	  
	    qiwle(k) = qiwle(k) + 0.5*w(i,j,k)*      &  ! ice water 
                ( qci(i,j,k-1)+qpi(i,j,k-1)-qiz(k-1) &
                 +qci(i,j,k)+qpi(i,j,k)-qiz(k))
...
!-----------------------------------------------------------
	...
	do k=1,nzm
	 ...
	 qi2z(k) = 0.
	 qs2z(k) = 0.
	 tl2z(k) = 0.                ! thetal variance  (MO)
	 tlz(k) = tlz(k)*factor_xy   ! mo
	 do j=1,ny 
	  do i=1,nx
	    ...
	    if(w(i,j,k)+w(i,j,k+1).gt.0) aup(k) = aup(k) + 1
  	    tl2z(k)=tl2z(k)+ &   ! thetal variance (MO)
             (tabs(i,j,k)*prespot(k)*(1.-fac_cond*(qcl(i,j,k)+qpl(i,j,k)) &
              /tabs(i,j,k)) -tlz(k))**2
	  end do
	 end do
	 ...
	 do j=1,ny
	  do i=1,nx
!mo	    qc2z(k) = qc2z(k)+(qcl(i,j,k)-qcz(k))**2
!mo	    qi2z(k) = qi2z(k)+(qci(i,j,k)-qiz(k))**2
	    qc2z(k) = qc2z(k)+(qcl(i,j,k)+qpl(i,j,k) & ! Include precip into liquid 
                               -qcz(k))**2             ! and
	    qi2z(k) = qi2z(k)+(qci(i,j,k)+qpi(i,j,k) & ! ice profiles for 
                               -qiz(k))**2             ! ISDAC (MO:mikhail@pnnl.gov)
         ...
         do ncond = 1,ncondavg
            ...
            do k=1,nzm
               ...
               !                   + (w(i,j,k)-w(i,jb,k))*dz/dy)
                            uwsubgrid = 0.         ! MO 4/13/16 temp fix
                            vwsubgrid = 0.         ! MO 4/13/16 temp fix
...
!MO            call hbuf_put('QN'//TRIM(condavgname(ncond)),qccl+qicl,1.e3)
	    tmp_out=qccl+qicl           !MO 4/13/16
            call hbuf_put('QN'//TRIM(condavgname(ncond)),tmp_out,1.e3)
...
!MO	call hbuf_put('THTEND',ttend-tlsvadv,86400.)
!MO	call hbuf_put('QHTEND',qtend-qlsvadv,86400.*1.e3)
        tmp_out=ttend-tlsvadv                         !MO 4/13/16
	call hbuf_put('THTEND',tmp_out,86400.)        !MO 4/13/16
        tmp_out=qtend-qlsvadv                         !MO 4/13/16
	call hbuf_put('QHTEND',tmp_out,86400.*1.e3)   !MO 4/13/16
...
!MO        call hbuf_put('URESID',ustor-factor_xy*(utendcor+uadv+udiff)-unudge-ulsvadv,1.)
!MO        call hbuf_put('VRESID',vstor-factor_xy*(vtendcor+vadv+vdiff)-vnudge-vlsvadv,1.)

        tmp_out=ustor-factor_xy*(utendcor+uadv+udiff)-unudge-ulsvadv  !MO 4/13/16
        call hbuf_put('URESID',tmp_out,1.)                            !MO 4/13/16
        tmp_out=vstor-factor_xy*(vtendcor+vadv+vdiff)-vnudge-vlsvadv  !MO 4/13/16
        call hbuf_put('VRESID',tmp_out,1.)                            !MO 4/13/16
...
!===================================================
! ISDAC ADDITIONS (MO: mikhail@pnnl.gov)

        w_max = 0.
        w_max = max(w_max,maxval(w(1:nx,1:ny,1:nz)))

        zinv= 0.        ! inversion height (thetal crossing 267 K)
        zctl= 0.        ! liquid cloud-top height (ql>0.0001 g/kg)
        zcbl= 0.        ! liquid cloud-base height (ql>0.0001 g/kg)
        lwpa= 0.        ! mean liquid water path
        iwpa= 0.        ! mean ice water path
        lwp_var= 0.     ! liquid water path variance
        iwp_var= 0.     ! ice water path variance
!        prl_sfc= 0.     ! liquid precipitation flux at the surface
!        pri_sfc= 0.     ! ice precipitation flux at the surface
!        prl_400= 0.     ! liquid precipitation flux at z=400m
