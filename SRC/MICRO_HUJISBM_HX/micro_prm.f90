module micro_prm
  ...
  ! DELZCCN - changing with heihgt

  ! if DELZCCN=0 then no vertical distribution concentration of aerosol

  ! PARAMETER (DELZCCN=0.0E0)
  ...
  ! flags for turbulance
  integer, parameter :: LIQTURB=1
  integer, parameter :: ICETURB=1

  ! parameter used for calculation of ice microphysics
  ...
  ! if ifreeze_top=1 homogeneous freezing at the top is included
  ! if ifreeze_top=0 no homogeneous freezing at the top
  ! PARAMETER (IFREEZE_TOP=1)

  ! Parameters used into algorithm of heterogenic freezing by Vali,1975:
  ! an0_freeze=10/cm3(Vali,1975), gamma=4.4
  real, parameter :: an0_freez=10.
  ! PARAMETER (gamma=4.4)

  ! KRFREEZ set bin boundary between plates and hail with freezing
  ! PARAMETER (KRFREEZ=21)
  ! AFREEZE, BFREEZE, BFREEZEMAX - coefficients in formula of homogeneous freezing
  ...
  ! TCRIT - temperature boundary between graupel and hail with
  ! coagulation
  real, parameter :: TCRIT=270.15E00

  ! if i_ww=1 then calculates only for water-water=water
