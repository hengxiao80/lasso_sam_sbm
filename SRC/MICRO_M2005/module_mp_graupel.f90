...
!  USE module_state_description
  ! parameters from SAM and options from wrapper routine.
   ...
   real, public :: &
        ...
        arctic_ice_nuclei_Nice, & ! If RHice > arc...rhi_thresh, then
        arctic_ice_nuclei_rhi_thresh !  Nice = arc..._Nice.
...
! ADD SEDIMENTATION TENDENCIES (UNITS OF KG/KG/S)
                            ...
                            NGSTEN,NRSTEN, NISTEN, NSSTEN, NCSTEN, & ! added here for consistency - Heng XIAO
...
! ADD SEDIMENTATION TENDENCIES
                                  QGSTEN,QRSTEN,QISTEN,QNISTEN,QCSTEN, &
                                  NGSTEN,NRSTEN,NISTEN,NSSTEN,NCSTEN) ! HX
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE M2005MICRO_GRAUPEL(QC3DTEN,QI3DTEN,QNI3DTEN,QR3DTEN,NC3DTEN,    &
        ...
        QGSTEN,QRSTEN,QISTEN,QNISTEN,QCSTEN, &
        NGSTEN, NRSTEN, NISTEN, NSSTEN, NCSTEN) ! adding output of number sedimentation tendencies - Heng XIAO 03/10/2026
...
! SEDIMENTATION TENDENCIES FOR NUMBER CONCENTRATION FOR OUTPUT - Heng XIAO, 03/10/2026

      REAL, DIMENSION(KMS:KME) ::  NGSTEN            ! GRAUPEL NUMBER SED TEND (1/KG/S)
      REAL, DIMENSION(KMS:KME) ::  NRSTEN            ! RAIN NUMBER SED TEND (1/KG/S)
      REAL, DIMENSION(KMS:KME) ::  NISTEN            ! CLOUD ICE NUMBER SED TEND (1/KG/S)
      REAL, DIMENSION(KMS:KME) ::  NSSTEN            ! SNOW NUMBER SED TEND (1/KG/S)
      REAL, DIMENSION(KMS:KME) ::  NCSTEN            ! CLOUD WAT NUMBER SED TEND (1/KG/S)
! OUTPUT VARIABLES
...
! INITIALIZE SEDIMENTATION TENDENCIES FOR NUMBER CONCENTRATION - HX
      NRSTEN(K) = 0.
      NISTEN(K) = 0.
      NSSTEN(K) = 0.
      NCSTEN(K) = 0.
...
! Skip if ice number is constrained (i.e., INUC = 1) (MO, 7 Jan 2010)
      IF (INUC.EQ.0) THEN
...
! limit to 500 L-1
      ...
      kc2=MAX(kc2/rho(k),0.)  ! convert to kg-1
          ...
          IF (T3D(K).LT.273.15.AND.QVQVSI(K).GT.arctic_ice_nuclei_rhi_thresh) THEN
!bloss          IF (T3D(K).LT.273.15.AND.QVQVSI(K).GT.1.05) THEN

             KC2 = arctic_ice_nuclei_Nice*1000./RHO(K)  ! CONVERT FROM L-1 TO KG-1 (SHEBA & ISDAC values)
!bloss             KC2 = 0.5*1000./RHO(K)  ! CONVERT FROM L-1 TO KG-1 (SHEBA & ISDAC values)
!             KC2 = 1.7*1000./RHO(K)  ! CONVERT FROM L-1 TO KG-1 (SHEBA & ISDAC values)
!             KC2 = 0.16*1000./RHO(K)  ! CONVERT FROM L-1 TO KG-1 (MPACE values)
          IF (KC2.GT.NI3D(K)+NS3D(K)+NG3D(K)) THEN
             NNUCCD(K) = (KC2-NI3D(K)-NS3D(K)-NG3D(K))/DT
             MNUCCD(K) = NNUCCD(K)*MI0
          END IF
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
      ...
      NCSTEN(K) = NCSTEN(K)-FALTNDNC/NSTEPC/RHO(k) ! HX
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
      ...
      NSSTEN(K) = NSSTEN(K)-FALTNDNS/NSTEPS/RHO(k) ! HX
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
      ...
      NGSTEN(K) = NGSTEN(K)-FALTNDNG/NSTEPG/RHO(k) ! HX
